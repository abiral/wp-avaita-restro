...
class Ava_Restro_Nepali_Date {
    ...
    public function get_next_nep_year_by_ad($timestamp) {
        $year   = date('Y', $timestamp);
        $month  = date('m', $timestamp);
        $day    = date('d', $timestamp);

        $nepali_date = $this->eng_to_nep($year, $month, $day);

        $year = $nepali_date['year'] + 1;
        $month = $nepali_date['month'];
        $day = $nepali_date['date'];

        $english_date = $this->nep_to_eng($year, $month, $day);

        return implode('-', array($english_date['year'], str_pad($english_date['month'], 2, '0', STR_PAD_LEFT), str_pad($english_date['date'], 2, '0', STR_PAD_LEFT)));
    }
