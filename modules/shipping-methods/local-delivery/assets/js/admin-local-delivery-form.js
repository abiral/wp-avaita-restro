+(function ($) {
  ...
  function blockUI() {
    tb_show(
      "",
      "#TB_inline?height=240&amp;width=405&amp;inlineId=avaita-loader&amp;modal=true",
      null
    );
  }
  ...
  function sync() {
    ...
    const handleSubmission = function () {
      ...
      if (proceededRecords > 0) {
        notice.showSuccess(
          messageLists.saved_refresh_to_see.replace(
            "{{number}}",
            proceededRecords
          )
        );
  ...
  $(".save-data").on("click", function (event) {
    ...
    const payload = {
      ...
      subarea: parentEl.find('input[name="subarea"]').val(),
      ...
      minimum_order_threshold: parentEl
        .find('input[name="minimum_order_threshold"]')
        .val(),
      minimum_free_delivery: parentEl
        .find('input[name="minimum_free_delivery"]')
        .val(),
  ...
  $(".edit-location").on("click", function (event) {
    ...
    $('input[name="subarea"]').val(parentEl.find("td.subarea").data("val"));
    ...
    $('input[name="minimum_order_threshold"]').val(
      parentEl.find("td.min-threshold").data("val")
    );
    $('input[name="minimum_free_delivery"]').val(
      parentEl.find("td.min-delivery").data("val")
    );
    $('input[name="delivery_price"]').val(
      parentEl.find("td.delivery-price").data("val")
    );
  });
